...
namespace AtmoData
{
	...
	while (!speciation_stream.IsEmpty())
	  {
	    ...
	    if (Sp_emis_names[l] != "COVNM" && Sp_emis_names[l] != "NMVOC")
	      {
  ...
  */
  ...
  /*!
    ...
    \param LUC land use coverage.
    \param LUC_idf mask defining the area of idf.
  ...
  */
    ...
    for (i = 0; i < Nx_luc; i++)
	  ...
	  if ( i_emep >= 0 && i_emep < Nx_emep && j_emep >= 0 && j_emep < Ny_emep
	      ...
	      else
		{
		  ...
		  if ( (LUC(j, i) >= 10 && LUC(j, i) <= 19) || LUC(j, i) == 21 || LUC(j, i) == 23 )
		    Ratio = Ratio_oth
		      / real( Ratio_urb * Nurb_emep(j_emep, i_emep)
			      + Ratio_for * Nfor_emep(j_emep, i_emep)
			      + Ratio_oth * Noth_emep(j_emep, i_emep) );
  ...
  template <class real>
  void FireToLatLon(const Data<int, 2, real>& LUC,
					const real Ratio_urb,
					const real Ratio_for,
					const real Ratio_oth,
					const Data<int, 2, real>& Nurb_emep,
					...
					const Data<int, 2, real>& Noth_emep,
					const Data<int, 2, real>& Ntot_polair,
					Data<real, 2, real>& EmisLiousse_OC,
					Data<real, 2, real>& EmisLiousse_EC,
					Data<real, 2, real>& Emis_out_OC,
					Data<real, 2, real>& Emis_out_EC)
  {
    int i, j;
	Emis_out_OC.Fill(0.0);
	Emis_out_EC.Fill(0.0);
	...
	real Delta_x_emep = (Nurb_emep[1](Nx_emep-1) - Nurb_emep[1](0)) / real(Nx_emep-1);
	real x_min_emep = x_min_center_emep - Delta_x_emep/2.0;
	real y_min_center_emep = Nurb_emep[0](0);
	real Delta_y_emep = (Nurb_emep[0](Ny_emep-1) - Nurb_emep[0](0)) / real(Ny_emep-1);
      ...
      for (j = 0; j < Ny_luc; j++)
	{
	  ...
	  int i_emep = int ((lon - x_min_emep) / Delta_x_emep);
	  int j_emep = int ((lat - y_min_emep) / Delta_y_emep);
	       ...
	       && i_polair >= 0 && i_polair < Nx && j_polair >= 0 && j_polair < Ny )
	    {
	      ...
	      if (LUC(j, i) == 0)
	      ...
	      else
			{
			  ...
			  Emis_out_OC(j_polair, i_polair) += Ratio*EmisLiousse_OC(j_emep, i_emep);
			  Emis_out_EC(j_polair, i_polair) += Ratio*EmisLiousse_EC(j_emep, i_emep);			  
			}
	    }
	}
      ...
      for (j = 0; j < Ny; j++)
		{
		  surface = Ntot_polair(j, i) * factor
			* cos(Ntot_polair[0](j) * ratio_pi);;
		  Emis_out_OC(j,i) /= surface;
		  Emis_out_EC(j,i) /= surface;
    ...
    template <class real>
  void GridLiousseCorrespondences(const Data<int, 2, real>& LUC,
			   Data<int, 2, real>& Nurb_emep,
			   Data<int, 2, real>& Nwat_emep,
			   Data<int, 2, real>& Nfor_emep,
			   Data<int, 2, real>& Noth_emep,
			   Data<int, 2, real>& Ntot_polair)
  {
    ...
    for (int i = 0; i < Nx_luc; i++)
      for (int j = 0; j < Ny_luc; j++)
	{
	  real lon = (x_min_luc + i * delta_x_luc); // rad
	  real lat = (y_min_luc + j * delta_y_luc); // rad
  ...
  template <class real>
  void ReadECOC(Date date, const vector<string>& Sp_emis_names,
		string input_directory, string input_file,
		const Data<real, 3>& MonthlyFactors,
		const Data<real, 3>& DailyFactors,
		Data<list<EmepCountryEmission<real> >, 4, real>& Emis_land,
		...
		string countrylatlon)
  {
    ...
    // Reads the country codes and country numbers of EMEP.
    ExtStream file_stream(input_file);
    vector<string> CountryCode;
    vector<int> CountryNumber;
    while (!file_stream.IsEmpty())
    ...
    // Reads the county numbers of input file and its location
    ...
    int i_lat,i_lon;
    string country_code;
    RegularGrid<real>GridY(Ny_emep);
    RegularGrid<real>GridX(Nx_emep);
	
    Data<int, 2, real> code(GridY, GridX);
    Data<int, 2, real> emepcode(GridY, GridX);
	
    ExtStream Streamloc(countrylatlon);
    if (!Streamloc.is_open())
      throw string("File ") + countrylatlon + " doesn't exist.";
    
    cout << "toto" << endl;
    while (has_element(Streamloc))
      {
	Streamloc.GetLine(line);
	cout << line << endl;
	v = split(line, ";");
	lat=to_num<real>(v[0]);
	lon=to_num<real>(v[1]);
	country_code=v[2];
	int i_lon = int ((lon - x_min_emep) / Delta_x_emep);
	int i_lat = int ((lat - y_min_emep) / Delta_y_emep);
	int n=0;
	while (CountryCode[n] != country_code)
	  n++;
	if (i_lon >= 0 && i_lon < Nx_emep && i_lat >=0 && i_lat < Ny_emep)
	  {
	    code(i_lat,i_lon)=CountryNumber[n];
	  }
      }
    cout << "tata" << endl;

	// Read correspondance to EMEP countries numbers
	//	int code1, code2;	
	//ExtStream Streamcode(countrycode);
	//for (int i = 0; i < Nx_emep; i++)
	//  for (int j = 0; j < Ny_emep; j++)
	//	emepcode(j,i)=0;
	//if (!Streamcode.is_open())
	//  throw string("File ") + countrycode + " doesn't exist.";
	
	//while (has_element(Streamcode))
	//  {
	//	Streamcode.GetLine(line);
	//	v = split(line, ";");
	//	code1=to_num<int>(v[0]);
	//	code2=to_num<int>(v[2]);
	//	for (int i = 0; i < Nx_emep; i++)
	//	  for (int j = 0; j < Ny_emep; j++)
	//		if(code(j,i)==code1)
	//		  emepcode(j,i)=code2;
    ...
    char annee[4];
    Data<real, 1> emis(GridSectors);

    sprintf(annee, "%d", year);
    
    for(int l = 0; l < Nsp_emis; l++)
      {
	string emis_file = input_directory + Sp_emis_names[l] + annee + ".csv";
	ExtStream EmepEmisStream(emis_file);
	if (!EmepEmisStream.is_open())
	  throw string("File ") + emis_file + " doesn't exist.";
	...
	while (has_element(EmepEmisStream))
	  {
	    ...
	    for(int s = 0; s < 10; s++)
	      emis(s)=to_num<real>(v[2+s]);

	    int i_lon = int ((lon - x_min_emep) / Delta_x_emep);
	    int i_lat = int ((lat - y_min_emep) / Delta_y_emep);

	    if (code(i_lat,i_lon) >= Ncountry_max)
	      throw string("Country code number ") + to_str(code(i_lat,i_lon))
	    ...
	    for(int s = 0; s < 10; s++)
	      {
		monthly = MonthlyFactors(code(i_lat,i_lon), s, date.GetMonth() - 1);
		daily = DailyFactors(code(i_lat,i_lon), s, day);
		// In water.
		if ((code(i_lat,i_lon) >= 30 && code(i_lat,i_lon) <= 35) || code(i_lat,i_lon) == 70)
		  Emis_water(l, i_lat, i_lon, s).
		    push_back(EmepCountryEmission<real>(emis(s) *
							monthly * daily /
							365, code(i_lat,i_lon)));
		// In land.
		else
		  Emis_land(l, i_lat, i_lon, s).
		    push_back(EmepCountryEmission<real>(emis(s) *
							monthly * daily /
							365, code(i_lat,i_lon)));
	      }
  ...
  template <class real>
  void ECOCToLatLon(const Data<int, 2, real>& LUC,
                    const Data<int, 2, real>& LUC_idf,
		    const real Ratio_urb,
		    const real Ratio_for,
		    const real Ratio_oth,
		    ...
		    Data<list<EmepCountryEmission<real> >, 4, real>& Emis_out)
  {
	...
	if(LUC_idf(j, i) == 0)
	 {
	  real lon = x_min_luc + i * delta_x_luc;
	  real lat = y_min_luc + j * delta_y_luc;
  ...
  template <class real>
  void Speciation(const vector<string>& Sp_emis_names,
			     string speciation_directory,
			     vector<string>& Sp_model_names,
			     Data<real, 3>& Species_factor)
  {
  ...
  template <class real>
  void ComputeVerticalDistribution1D(const Data<real,1>& vertical_distribution_in,
				   const RegularGrid<real>& GridZ_interf_in,
				   const RegularGrid<real>& GridZ_interf_out,
				   Data<real, 1>& vertical_distribution_out)
  {
    ...
    // Vertical Grids.
    int Nz_in = vertical_distribution_in[0].GetLength();
    int Nz_out = vertical_distribution_out[0].GetLength();
    ...
    for (k = 0; k < Nz_out && k_in < Nz_in; k++)
      {
	if (GridZ_interf_out(k+1) <= GridZ_interf_in(k_in+1))
	    vertical_distribution_out(k) =
	      vertical_distribution_in(k_in) / DeltaZ_in(k_in)
	      * DeltaZ_out(k);
	else
	  {
	      vertical_distribution_out(k) =
  ...
  */
  ...
  template <class real>
  void DividesByHeights1D(const RegularGrid<real>& GridZ_interf_out,
			Data<real, 1>& vertical_distribution_out)
  {
    ...
    for (k = 0; k < Nz_out; k++)
      vertical_distribution_out(k) /= DeltaZ_out(k);
  }
