...
//! Default constructor.
  template <class T>
  LonlatToRotatedLonlat<T>::LonlatToRotatedLonlat(T theta,
						  T phi) throw():
  ...
  /*!
    \param lon longitude.
    \param lat latitude.
    \param lon_rot longitude in rotated coordinate.
    \param lat_rot latitude in rotated coordinate.
  */
  template <class T>
  void LonlatToRotatedLonlat<T>:: operator() (T lon, T lat,
					      T& lon_rot, T& lat_rot)
  {
    double x, y, z;
    double x_rot, y_rot, z_rot;

    x = cos(lon * pi_ / 180) * cos(lat * pi_ / 180) * Earth_radius_;
    y = sin(lon * pi_ / 180) * cos(lat * pi_ / 180) * Earth_radius_;
    z = sin(lat * pi_ / 180) * Earth_radius_;

    x_rot = (cos(theta_ * pi_ / 180) * cos(phi_ * pi_ / 180) * x)
      - (cos(theta_ * pi_ / 180) * sin(phi_ * pi_ / 180) * y)
      - (sin(theta_ * pi_ / 180) * z);

    y_rot = (sin(phi_ * pi_ / 180) * x) + (cos(phi_ * pi_ / 180) * y);
    
    z_rot = (sin(theta_ * pi_ / 180) * cos(phi_ * pi_ / 180) * x)
      - (sin(theta_ * pi_ / 180) * sin(phi_ * pi_ / 180) * y)
      + (cos(theta_ * pi_ / 180) * z);

    lat_rot = asin(z_rot / Earth_radius_) * 180 / pi_;
    lon_rot = atan2(y_rot, x_rot) * 180 / pi_; 

  }
    
//! Default constructor.
  template <class T>
  RotatedLonlatToLonlat<T>::RotatedLonlatToLonlat(T theta,
						  T phi) throw():
    Earth_radius_(6370997.), pi_(3.14159265358979323846264),
    theta_(theta), phi_(phi)
  {
  ...
  */
  template <class T>
  void RotatedLonlatToLonlat<T>:: operator() (T lon_rot, T lat_rot,
					      T& lon, T& lat)
  {
    double x, y, z;
    double x_rot, y_rot, z_rot;

    x_rot = cos(lon_rot * pi_ / 180) * cos(lat_rot * pi_ / 180) * Earth_radius_;
    y_rot = sin(lon_rot * pi_ / 180) * cos(lat_rot * pi_ / 180) * Earth_radius_;
    z_rot = sin(lat_rot * pi_ / 180) * Earth_radius_;

    x = (cos(theta_ * pi_ / 180) * cos(phi_ * pi_ / 180) * x_rot)
      + (sin(theta_ * pi_ / 180) * sin(phi_ * pi_ / 180) * z_rot)
      + (sin(phi_ * pi_ / 180) * y_rot);

    z = (-sin(theta_ * pi_ / 180) * x_rot) + (cos(theta_ * pi_ / 180) * z_rot);
    
    y = (-cos(theta_ * pi_ / 180) * sin(phi_ * pi_ / 180) * x_rot)
      - (sin(theta_ * pi_ / 180) * sin(phi_ * pi_ / 180) * z_rot)
      + (cos(theta_ * pi_ / 180) * y_rot);

    lat = asin(z / Earth_radius_) * 180 / pi_;
    lon = atan2(y, x) * 180 / pi_; 
