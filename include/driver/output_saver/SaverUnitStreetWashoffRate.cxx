...
#ifndef POLYPHEMUS_FILE_OUTPUT_SAVER_SAVERUNITSTREETWASHOFFRATE_CXX


#include "SaverUnitStreetWashoffRate.hxx"
...
namespace Polyphemus
{
  ...
  SaverUnitStreetWashoffRate<T, ClassModel>
  ::SaverUnitStreetWashoffRate():
  ...
  //! Destructor.
  template<class T, class ClassModel>
  SaverUnitStreetWashoffRate<T, ClassModel>::~SaverUnitStreetWashoffRate()
  {
  }
  ...
  //! Type of saver.
  template<class T, class ClassModel>
  string SaverUnitStreetWashoffRate<T, ClassModel>::GetType()  const
  {
    return type;
  }
  ...
  */
  template<class T, class ClassModel>
  void SaverUnitStreetWashoffRate<T, ClassModel>::Init(ConfigStream& config_stream, ClassModel& Model)
  {

    BaseSaverUnit<T, ClassModel>::Init(config_stream, Model);

    // Check that memory has been allocated for StreetConcentration.
    if(!Model.HasField("StreetWashoffRate"))
    ...
    Nstreet = Model.D2("StreetWashoffRate").GetLength(1);
        ...
        for (s = 0; s < this->Ns; s++)
          for (st = 0; st < Nstreet; st++)
            Concentration_(s, st) = 0.5
              * Model.D2("StreetWashoffRate")(this->species_index[s], st);
  ...
  */
  template<class T, class ClassModel>
  void SaverUnitStreetWashoffRate<T, ClassModel>::InitStep(ClassModel& Model)
  {
  ...
  */
  template<class T, class ClassModel>
  void SaverUnitStreetWashoffRate<T, ClassModel>::Save(ClassModel& Model)
  {
    if (this->averaged)
      {
        ...
        for (int s = 0; s < this->Ns; s++)
          {
            ...
            for(int st = 0; st < Nstreet; st++)
              Concentration_street(st) = Model.D2("StreetWashoffRate")(this->species_index[s], st);
...
#define POLYPHEMUS_FILE_OUTPUT_SAVER_SAVERUNITSTREETWASHOFFRATE_CXX
#endif
