...
#ifndef POLYPHEMUS_FILE_OUTPUT_SAVER_SAVERUNITSTREETDRYDEPOSITIONVELOCITY_CXX


#include "SaverUnitStreetDryDepositionVelocity.hxx"
...
namespace Polyphemus
{
  ...
  SaverUnitStreetDryDepositionVelocity<T, ClassModel>
  ::SaverUnitStreetDryDepositionVelocity():
  ...
  //! Destructor.
  template<class T, class ClassModel>
  SaverUnitStreetDryDepositionVelocity<T, ClassModel>::~SaverUnitStreetDryDepositionVelocity()
  {
  }
  ...
  //! Type of saver.
  template<class T, class ClassModel>
  string SaverUnitStreetDryDepositionVelocity<T, ClassModel>::GetType()  const
  {
    return type;
  }
  ...
  */
  template<class T, class ClassModel>
  void SaverUnitStreetDryDepositionVelocity<T, ClassModel>::Init(ConfigStream& config_stream,
                                            ClassModel& Model)
  {

    BaseSaverUnit<T, ClassModel>::Init(config_stream, Model);

    // Check that memory has been allocated for StreetDryDepositionVelocityConcentration.
    if(!Model.HasField("StreetDryDepositionVelocity"))
      throw string("The model does not have the array for street dry deposition velocities.");
    ...
    Nstreet = Model.D2("StreetDryDepositionVelocity").GetLength(1);
  ...
  */
  template<class T, class ClassModel>
  void SaverUnitStreetDryDepositionVelocity<T, ClassModel>::InitStep(ClassModel& Model)
  {
  ...
  */
  template<class T, class ClassModel>
  void SaverUnitStreetDryDepositionVelocity<T, ClassModel>::Save(ClassModel& Model)
  {
    if (this->averaged)
      {
        ...
        for (int s = 0; s < this->Ns; s++)
          {
            ...
            for(int st = 0; st < Nstreet; st++)
              Concentration_street(st) = Model.D2("StreetDryDepositionVelocity")(this->species_index[s], st);
...
#define POLYPHEMUS_FILE_OUTPUT_SAVER_SAVERUNITSTREETDRYDEPOSITIONVELOCITY_CXX
#endif
