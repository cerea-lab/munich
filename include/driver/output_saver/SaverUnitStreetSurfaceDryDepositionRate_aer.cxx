...
#ifndef POLYPHEMUS_FILE_OUTPUT_SAVER_SAVERUNITSTREETSURFACEDRYDEPOSITIONRATE_AER_CXX


#include "SaverUnitStreetSurfaceDryDepositionRate_aer.hxx"
...
namespace Polyphemus
{
  ...
  SaverUnitStreetSurfaceDryDepositionRate_aer<T, ClassModel>
  ::SaverUnitStreetSurfaceDryDepositionRate_aer():
  ...
  //! Destructor.
  template<class T, class ClassModel>
  SaverUnitStreetSurfaceDryDepositionRate_aer<T, ClassModel>::~SaverUnitStreetSurfaceDryDepositionRate_aer()
  {
  }
  ...
  //! Type of saver.
  template<class T, class ClassModel>
  string SaverUnitStreetSurfaceDryDepositionRate_aer<T, ClassModel>::GetType()  const
  {
  ...
  */
  template<class T, class ClassModel>
  void SaverUnitStreetSurfaceDryDepositionRate_aer<T, ClassModel>::Init(ConfigStream& config_stream,
						ClassModel& Model)
  {
    ...
    if(!Model.HasField("StreetSurfaceDryDepositionRate_aer"))
    ...
    Nstreet = Model.D3("StreetSurfaceDryDepositionRate_aer").GetLength(2);
	...
	for (b = 0; b < int(species_list_aer[s].second.size()); b++)
	  {
		...
		* Model.D3("StreetSurfaceDryDepositionRate_aer")(base_s, base_b, st);
  ...
  */
  template<class T, class ClassModel>
  void SaverUnitStreetSurfaceDryDepositionRate_aer<T, ClassModel>::InitStep(ClassModel& Model)
  {
  ...
  */
  template<class T, class ClassModel>
  void SaverUnitStreetSurfaceDryDepositionRate_aer<T, ClassModel>::Save(ClassModel& Model)
  {
    int s, b, k, j, i, st;
    // Indices in the model.
    int base_s, base_b;
	  ...
	  for (b = 0; b < int(species_list_aer[s].second.size()); b++)
	    {
	      ...
	      //for (st = 0; st < Nstreet; st++)
	      //	{
	      Data<T, 1> Concentration_tmp(&Model.D3("StreetSurfaceDryDepositionRate_aer")
					   (base_s, base_b,0),
					   shape(Nstreet));
...
#define POLYPHEMUS_FILE_OUTPUT_SAVER_SAVERUNITSTREETSURFACEDRYDEPOSITIONRATE_AER_CXX
#endif
