...
#ifndef POLYPHEMUS_FILE_OUTPUT_SAVER_SAVERUNITSTREETSURFACEDEPOSITEDMASS_AER_CXX


#include "SaverUnitStreetSurfaceDepositedMass_aer.hxx"
...
namespace Polyphemus
{
  ...
  SaverUnitStreetSurfaceDepositedMass_aer<T, ClassModel>
  ::SaverUnitStreetSurfaceDepositedMass_aer():
  ...
  //! Destructor.
  template<class T, class ClassModel>
  SaverUnitStreetSurfaceDepositedMass_aer<T, ClassModel>::~SaverUnitStreetSurfaceDepositedMass_aer()
  {
  }
  ...
  //! Type of saver.
  template<class T, class ClassModel>
  string SaverUnitStreetSurfaceDepositedMass_aer<T, ClassModel>::GetType()  const
  {
  ...
  */
  template<class T, class ClassModel>
  void SaverUnitStreetSurfaceDepositedMass_aer<T, ClassModel>::Init(ConfigStream& config_stream,
						ClassModel& Model)
  {
    ...
    if(!Model.HasField("StreetSurfaceDepositedMass_aer"))
    ...
    Nstreet = Model.D3("StreetSurfaceDepositedMass_aer").GetLength(2);
	...
	for (b = 0; b < int(species_list_aer[s].second.size()); b++)
	  {
		...
		* Model.D3("StreetSurfaceDepositedMass_aer")(base_s, base_b, st);
  ...
  */
  template<class T, class ClassModel>
  void SaverUnitStreetSurfaceDepositedMass_aer<T, ClassModel>::InitStep(ClassModel& Model)
  {
  ...
  */
  template<class T, class ClassModel>
  void SaverUnitStreetSurfaceDepositedMass_aer<T, ClassModel>::Save(ClassModel& Model)
  {
    int s, b, k, j, i, st;
    // Indices in the model.
    int base_s, base_b;
	  ...
	  for (b = 0; b < int(species_list_aer[s].second.size()); b++)
	    {
	      base_s = Model.GetSpeciesIndex_aer(species_list_aer[s].first);
	      base_b = species_list_aer[s].second[b];	      
	      Data<T, 1> Concentration_tmp(&Model.D3("StreetSurfaceDepositedMass_aer")
					   (base_s, base_b,0),
					   shape(Nstreet));
...
#define POLYPHEMUS_FILE_OUTPUT_SAVER_SAVERUNITSTREETSURFACEDEPOSITEDMASS_AER_CXX
#endif
