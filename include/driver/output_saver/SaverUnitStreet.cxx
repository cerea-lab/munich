...
#ifndef POLYPHEMUS_FILE_OUTPUT_SAVER_SAVERUNITSTREET_CXX
...
namespace Polyphemus
{
  ...
  //! Destructor.
  template<class T, class ClassModel>
  SaverUnitStreet<T, ClassModel>::~SaverUnitStreet()
  {
  }
  ...
  //! Type of saver.
  template<class T, class ClassModel>
  string SaverUnitStreet<T, ClassModel>::GetType()  const
  {
    return type;
  }
  ...
  */
  template<class T, class ClassModel>
  void SaverUnitStreet<T, ClassModel>::Init(ConfigStream& config_stream,
                                            ClassModel& Model)
  {
        ...
        for (s = 0; s < this->Ns; s++)
          for (st = 0; st < Nstreet; st++)
            Concentration_(s, st) = 0.5
              * Model.GetStreetConcentration()(this->species_index[s], st);
      }
  ...
  */
  template<class T, class ClassModel>
  void SaverUnitStreet<T, ClassModel>::InitStep(ClassModel& Model)
  {
  ...
  */
  template<class T, class ClassModel>
  void SaverUnitStreet<T, ClassModel>::Save(ClassModel& Model)
  {
    if (this->averaged)
      {
        ...
        for (int s = 0; s < this->Ns; s++)
          {
            ...
            for(int st = 0; st < Nstreet; st++)
              Concentration_street(st) = Model.GetStreetConcentration()(this->species_index[s], st);
...
#define POLYPHEMUS_FILE_OUTPUT_SAVER_SAVERUNITSTREET_CXX
#endif
